...
// Compile make -f Makefile_manless_dapoer_pandan_wangi

static char app_version[] = "2.0.1";
static char app_builddate[] = "31/08/2024";
...
int 
store_local(char *waktumasuk, char *posmasuk, char *kasirmasuk, char *jeniskend, char *noid, char *jnslangganan, int shift, char *notiket, char *nopol, char *nokartu)
{
    sqlite3 *db;
    char *err_msg = 0;
    char *v19;
    ...
    snprintf(sql, sizeof(sql), "INSERT INTO tlog(waktumasuk, posmasuk, kasirmasuk, jeniskend, noid, jnslangganan, shift, notiket, nopol, nokartu) VALUES(\"%s\", \"%s\", \"%s\", \"%s\", \"%s\", \"%s\", \"%s\", \"%s\", \"%s\", \"%s\");",
             waktumasuk, posmasuk, kasirmasuk, jeniskend, noid, jnslangganan, shiftt, notiket, nopol, nokartu);
    // printf("[SQL] %s\n", sql);
...
create_ticket()
{
    ...
    asprintf(&v4, "[INFO] Tipe [0:Kartu|1:Tiket] -> %d\n", typeTrans);
    app_log(v4);
    free(v4);
    printf("[INFO] Tipe [0:Kartu|1:Tiket] -> %d\n", typeTrans);
    if(typeTrans == 1) {
        sprintf(nokartu, "%s", "-");
    }
        ...
        if (res != CURLE_OK)
        {
            ...
            if (json == NULL) {
                ...
                if (ret == 1) {
                    ...
                    // save to database
                    store_local((char *)waktumasuk, (char *)id_manless, (char *)gate_name, (char *)gate_kind, (char *)noid, (char *)jenis_langganan, shift, (char *)notiket, (char *)nopol, (char *)nokartu);
                    // print the ticket
                    if (typeTrans == 1) {
...
main(int argc, char *argv[])
{
    ...
    signal(SIGSEGV, handle_signt);
    signal(SIGABRT, handle_signt);
