...
// Update version list:
// 1.0.0 === Basic version
// 1.0.1 === Add e-money method

static char app_version[] = "1.0.0";
static char app_builddate[] = "02/01/2024";
static int g_run = 1;
...
int insert_image(char *file)
{
	...
	if (curl)
	{
		...
		if (res != CURLE_OK)
		{
			fprintf(stderr, "[ERROR][API IMAGE] Tidak dapat terhubung ke API! -> %s\n", curl_easy_strerror(res));
		}
...
int store_cloud(char *waktumasuk, char *posmasuk, char *kasirmasuk, char *jeniskend, char *noid, char *jnslangganan, char *shift, char *notiket, char *nopol, char *nokartu)
{
	...
	if (curl)
	{
		...
		if (res != CURLE_OK)
		{
			fprintf(stderr, "[ERROR][API] curl_easy_perform() failed: %s\n", curl_easy_strerror(res));
		}
	...
	else
	{
		while ((entry = readdir(dir)) != NULL)
		{
			if (entry->d_type == DT_REG)
			{
				...
				{
					printf("[INFO] Migrate => %s  %d/%d Failed\n", source, i + 1, len);
				}
				i++;
			}
		}
...
void *migrate_transaction(void *tipe)
{
	...
	while (1)
	{
		...
		while (sqlite3_step(res) == SQLITE_ROW)
		{
			...
			store_cloud((char *)waktu_masuk, (char *)pos, (char *)gate_name, (char *)jenis_kendaraan, (char *)noid, (char *)jnslangganan, (char *)shift, (char *)ticket, (char *)nopol, (char *)kartu);

			printf("[INFO] Migrate transaction => ID:[%d] | TIKET:[%s] | WAKTU MASUK:[%s] | JENIS KENDARAAN:[%s] %d/%d Ok\n",
...
void controller(char *port, int baudrate, char *xstr)
{
	...
	while (1)
	{
		...
		if (length == -1)
		{
			printf("[ERROR][CONTROLLER] Error reading from serial port\n");
		}
		else if (length == 0)
		{
		...
		else
		{
			...
			if (buffer[0] == 'O' && buffer[1] == 'P')
			{
				...
				if (buffer[6] == '2' && buffer[7] == 'T')
				{
					...
					{
						printf("[INFO] Harap tekan tombol tiket terlebih dahulu!\n");
					}
				}
			}
		}
...
int main(int argc, char *argv[])
{
	...
	while (sqlite3_step(res) == SQLITE_ROW)
	{
		...
		sprintf(ip_camera_1, "%s", sqlite3_column_text(res, 15));
		sprintf(ip_camera_1_user, "%s", sqlite3_column_text(res, 16));
		sprintf(ip_camera_1_password, "%s", sqlite3_column_text(res, 17));
		sprintf(ip_camera_1_model, "%s", sqlite3_column_text(res, 18));
		sprintf(ip_camera_1_status, "%s", sqlite3_column_text(res, 19));
		sprintf(ip_camera_2, "%s", sqlite3_column_text(res, 20));
		sprintf(ip_camera_2_user, "%s", sqlite3_column_text(res, 21));
		sprintf(ip_camera_2_password, "%s", sqlite3_column_text(res, 22));
		sprintf(ip_camera_2_model, "%s", sqlite3_column_text(res, 23));
		sprintf(ip_camera_2_status, "%s", sqlite3_column_text(res, 24));
		sprintf(lpr_engine, "%s", sqlite3_column_text(res, 25));
		sprintf(lpr_engine_status, "%s", sqlite3_column_text(res, 26));
		sprintf(face_engine, "%s", sqlite3_column_text(res, 27));
		sprintf(face_engine_status, "%s", sqlite3_column_text(res, 28));
		sprintf(nama_lokasi, "%s", sqlite3_column_text(res, 29));
		sprintf(alamat_lokasi, "%s", sqlite3_column_text(res, 30));
		sprintf(pesan_masuk, "%s", sqlite3_column_text(res, 31));
		sprintf(status_config, "%s", sqlite3_column_text(res, 32));
		count++;
	}
	// printf("Rows count: %d\n", count);
	sqlite3_finalize(res);
